...
public class Atm {
    ...
    public static void main(String[] args) {
        ...
        int[] cumulativeSum = new int[num];
        cumulativeSum[0] = time[0];

        IntStream.range(1, num).forEach(i -> cumulativeSum[i] = cumulativeSum[i - 1] + time[i]);

        int sum = Arrays.stream(cumulativeSum).sum();
